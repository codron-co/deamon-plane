...
0. **Site yapılandırması Plane'den** ([ADR-12](decisions/adr-12-site-config-from-plane.md)): müşteri site env'i yalnızca önyükleme anahtarları (`APP_KEY`, `DB_PASSWORD`, `MYSQL_ROOT_PASSWORD`, `CONTROL_PLANE_AGENT_SECRET`, `CONTROL_PLANE_HOST_ALLOWLIST`, `DEAMON_CHANNEL`); diğer her ayar imzalı agent ile site DB'sine; güvenlik tabanı CMS kodunda; yeni env anahtarı yok.
