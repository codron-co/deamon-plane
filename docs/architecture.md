...
8. Deploy visibility (Task 6): signed `POST /webhooks/coolify` updates `deployments.status` (same rows as `PollDeploymentJob`). Fleet home KPIs: total sites, by channel, unhealthy (`status=error` or agent fail/stale), failed deploys, deploying. HMAC scheme: [modules/coolify-webhooks.md](modules/coolify-webhooks.md). GitHub theme webhooks: [modules/github-webhooks.md](modules/github-webhooks.md).
