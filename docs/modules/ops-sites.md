...
- Work Plane itself finishes (`sites.live_sync`, `sites.coolify_sync`, `coolify.inventory_sync`, `themes.catalog_sync`, `sites.bulk_compose`, `sites.bulk_auto_deploy`, `sites.bulk_publish_status`): counts from `ops.bulk.result*` (`1 tamam`) and status `Bitti`.
