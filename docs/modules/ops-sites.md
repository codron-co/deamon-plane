...
| POST | `/sites/{site}/follow-head` | `ops.sites.follow-head` | operator, super_admin; `git_commit_sha: HEAD` + auto-deploy on (off while the site is on the CI gate) + deploy |
| POST | `/sites/{site}/deploy-gate` | `ops.sites.deploy-gate` | operator, super_admin; `gate=coolify\|ci` — [CI-gated rollout](ci-gated-rollout.md); never deploys |
| POST | `/sites/{site}/deploy-canary` | `ops.sites.deploy-canary` | operator, super_admin; canary flag for CI rollouts |
| POST | `/sites/bulk/deploy-gate` | `ops.sites.bulk.deploy-gate` | operator, super_admin; deploy gate for selected or `all=1` |
...
| POST | `/sites/{site}/admins/{remoteAdmin}/password-invite` | `ops.sites.admins.password-invite` | operator, super_admin; resend set-password mail (CMS 1.2.18+) |
...
- Work Plane itself finishes (`sites.live_sync`, `sites.coolify_sync`, `coolify.inventory_sync`, `themes.catalog_sync`, `sites.bulk_compose`, `sites.bulk_auto_deploy`, `sites.bulk_deploy_gate`, `sites.bulk_publish_status`, `domains.bulk_bind`): counts from `ops.bulk.result*` (`1 tamam`) and status `Bitti`. `domains.bulk_bind` is here on purpose: `setDomains` is a PATCH, not a rebuild.
...
**Deploy failure diagnosis.** Every `deployments` row that turns `failed` (poll, webhook, sync, provision, channel switch) is classified by `DeploymentFailureClassifier` from `DiagnoseDeploymentJob` (model `saved` hook, after commit, once per row). Codes whose evidence lives inside a container (`mysql_exited`, `redis_exited`, `app_exited`, `oom_killed`, `migration_failed`, …) pull `GET /applications/{uuid}/logs`, redact secrets, keep an 8 KB tail on `deployments.diagnosis.container_logs` and classify again, so the compose symptom (`dependency failed to start`) becomes the real cause (`mysql_no_root_password`, `mysql_data_newer_version`, `mysql_locked`, `disk_full`, …). One safe automatic fix per code (`AUTO_SAFE`: `sync_env`, `redeploy`, `restart_app`, `sync_deployments`, `bind_domains`) runs through `SiteAppHealthFixer` unless disabled (`ops.diagnosis.auto_fix`), a newer deployment exists, or the same fix already ran for this site inside `repeat_window_hours` (loop guard); the outcome is stored as `diagnosis.auto_fix` and audited as `site.deploy_auto_fix`. Deploy detail shows the **Teşhis** card (cause, evidence line, what Plane did, operator steps, server commands with the container name filled in, container log, fix buttons via `POST /sites/{site}/app-health/fix`) and **Teşhisi yenile** (`POST /sites/{site}/deployments/{deployment}/diagnose`, re-reads logs, never re-runs the fix). The list row, the jobs widget and the pasteable report lead with the diagnosis title. App health maps a diagnosed latest failure to `deploy_diagnosed` (key = code, fix = the code's first fix) instead of the generic `deploy_failed`. Wording: `lang/{tr,en}/deploy_diagnosis.php`; catalogue and manual paths: [../runbooks/deploy-failure-triage.md](../runbooks/deploy-failure-triage.md). Tests: `DeploymentFailureClassifierTest`, `DeploymentDiagnosisTest`.

**Container down (placeholder page).** A site whose app container crashed under `restart: no` shows the CodRon *Your domain is ready* page while the last deploy reads `finished`. The agent health poll recognises that page (`AgentHealthReason::ProxyFallback`, `SiteAgentClient::isProxyFallbackPage`) and App health reports `app_not_running` with **Yeniden başlat** (`restart_app` → `POST /applications/{uuid}/restart`) instead of `agent_unhealthy`; live Coolify inspect adds the same issue when the app status contains `exited` / `degraded` / `restarting` (not for `stopped` sites). New fixes in `SiteAppHealthFixer::FIX_ORDER`: `sync_deployments` (pull rows from Coolify + recover an `error` site whose latest is finished), `restart_app`, `stop_then_redeploy`, `rollback_last_good` (pin the newest finished `commit_sha`; operator-only), `follow_head`.

**Stale core theme.** CMS 1.2.30+ health reports `core_theme.in_sync`; `false` means `themes/default` on the site volume is older than the image, so core:: pages (timeline, appointment confirm…) can 500. App health shows `core_theme_stale` with **Yeniden başlat**, and the health poll restarts the app once per 6 h on its own (`CoreThemeHealer`) — the CMS entrypoint refreshes the copy on boot. See [agent-client.md](agent-client.md).
