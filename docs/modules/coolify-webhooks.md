...
- Fleet dashboard KPIs (6): total sites, by channel, unhealthy (`status=error` **or** agent timeout / bad signature / `queue_ok=false` / stale), failed deploys, deploying, agent secret. `needs_secret` does not count. Unhealthy is SQL (`Site::unhealthy()`), not a PHP walk of every site. See [agent-client.md](agent-client.md).
- **Failed deploys is a window, not a lifetime total.** The card counts *sites* with a failure inside `ops.fleet.failed_deploy_window_hours` (default 24, label and hint state the window), judged by `COALESCE(finished_at, started_at, created_at)` so a build that started days ago and failed an hour ago still counts. Retries of one site are one problem: the count is `count(distinct site_id)` and the attention list shows the **latest failure per site**, newest first, so the number and the list below it cannot disagree. Both attention lists cap at `ops.fleet.attention_limit` (default 8) and account for the remainder with `fleet.attention.more` (`+22 site daha`); the chip above each list always reports the real total. `FleetDashboardKpis` memoises the unhealthy SQL set so the KPI and the attention list do not query twice. Tests: `FleetFailedDeployWindowTest`, `FleetDashboardTest`, `HealthAppFilterTest`.
