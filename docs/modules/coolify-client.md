...
Existing Coolify app is **PATCH** only for pack migrate, auto-deploy, pin, follow HEAD, and channel switch. Those paths never DELETE / `delete_volumes`. Redeploy is `POST /deploy?force=true` only (no PATCH, no DELETE).
...
| Redeploy | `POST /deploy?uuid=&force=true` | Rebuild current pin or HEAD. No PATCH. Single + selected bulk. |
