...
Existing Coolify app is **PATCH** only for pack migrate, auto-deploy, pin, follow HEAD, and channel switch. Those paths never DELETE / `delete_volumes`. Redeploy is `POST /deploy?force=true` (no application PATCH, no DELETE). Every Plane deploy also runs env sync (`GET /envs` + `PATCH /envs/bulk` + `DELETE` for catalog leftovers). Coolify-injected `SERVICE_*` / `COOLIFY_*` / `SOURCE_COMMIT` / `APP_URL` / `DEAMON_SITE_HOST` are never written or deleted.
...
| Env sync (every deploy; catalog = bootstrap keys only, [ADR-12](../decisions/adr-12-site-config-from-plane.md)) | `GET /applications/{uuid}/envs` then `PATCH .../envs/bulk` (+ `DELETE .../envs/{envUuid}` leftovers) | `CoolifyAppEnvSync` vs the **channel catalog**. Credentials: Settings → **Deamon Git** only (not Themes). Upserts catalog keys; deletes Coolify keys that are neither in the catalog nor Coolify-managed (`SERVICE_*`, `COOLIFY_*`, `SOURCE_COMMIT`, `APP_URL`, `DEAMON_SITE_HOST`). `CoolifyEnvCatalogSync` reads `.env.production.example` from the CMS repo; tokens `{{generated}}` / `{{site.*}}` / `{{plane.host}}` / empty / static; `#@secret`. Refresh: CMS `push` webhook, hourly `ops:sync-env-catalog`, Settings **Refresh from GitHub**, on-demand when empty. Live secrets are never rotated. Compose constants stay in `docker-compose.coolify.yml`. Never log values. |
