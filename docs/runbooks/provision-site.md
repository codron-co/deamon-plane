...
5. Job creates a Coolify application:
   ...
   - Branch = site channel
   - Coolify environment **name** defaults to `main` (1:1 with the main channel). Leftover connection name `production` / `prod` is rewritten to `main`. Laravel `APP_ENV` today follows the channel map (`production` / `staging` / `local`); per [ADR-12](../decisions/adr-12-site-config-from-plane.md) it becomes `production` for every live site (open item F-G-03).
