...
6. (Config note, [ADR-12](../decisions/adr-12-site-config-from-plane.md): a channel is only the git branch. Today the next deploy's env sync also writes the channel-mapped `APP_ENV`; target is `APP_ENV=production` on every live site.) Job calls Coolify `PATCH /applications/{uuid}` with `{ git_branch, git_commit_sha: "HEAD" }` only (follow the new branch tip). Empty `git_commit_sha` fails Coolify validation. Do **not** send `environment_uuid` — live Coolify 4.x returns `environment_uuid: This field is not allowed` and skips the branch change. Writes `APP_ENV` + `DEAMON_CHANNEL` via `/envs/bulk`, then `POST /deploy`. No `DELETE /applications/{uuid}`.
